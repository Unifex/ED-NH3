...
var HUD = {
  ...
  'create' : function(container) {
    ...
    credits = '';
    if (Ed3d.credits) {
      credits += '<h2>Credits</h2><div id="credits">';
      for (var i = 0; i < Ed3d.credits.length; i++) {
        credits += '<a href="' + Ed3d.credits[i].url + '" target="_blank">' + Ed3d.credits[i].name + '</a>';
        if (i < Ed3d.credits.length - 1) {
          credits += ' | ';
        }
      }
      credits += '</div>';
    ...
    about = '';
    if (Ed3d.about) {
      about += '<h2>About</h2><div id="about">' + Ed3d.about + '</div>';
    }
    kofi = '';
    if (Ed3d.kofi) {
      kofi += '<div id="kofi">' + Ed3d.kofi + '</div>';
    }
